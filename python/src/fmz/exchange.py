...
def httpPostWithSign(url, cmds, api_key, api_secret):
    ...
    req = urllib.request.Request(
        url,
        urllib.urlencode({'cmds': s_cmds, 'apikey': api_key, 'sign': sign}),
        headers=headers,
    )
    response = urllib.request.urlopen(req)
    return json.loads(response.read())
...
class MyExchange:
    ...
    def GetTicker(symbol):
        ...
        if 'error' in raw_data.keys():
            return {
                'error': json.dumps(
                    raw_data['error'], encoding="utf8", ensure_ascii=False
                )
            }
        ret_data = {
            "data": {
                "time": raw_data['result']['timestamp'],
                "buy": raw_data['result']['buy'],
                "sell": raw_data['result']['sell'],
                "last": raw_data['result']['last'],
                "high": raw_data['result']['high'],
                "low": raw_data['result']['low'],
    ...
    def GetDepth(symbol):
        ...
        ret_data = {
            "data": {"time": raw_data['result']['update_time'], "asks": [], "bids": []}
        }
    ...
    def GetRecords(symbol, period):
        url = (
            MyExchange.market_url
            + "?cmd=kline&size=200&period=%s&pair=" % MyExchange.kline_period[period]
            + symbol
        )
        raw_data = httpGet(url)
        if 'error' in raw_data.keys():
    ...
    def GetOrder(api_key, api_secret, orders_id):
        ...
        ret_data = {
            "data": {
                ...
                "type": (
                    "buy"
                    if raw_data['result'][0]['result']['order_side'] == 1
                    else "sell"
                ),
                "avg_price": 0,
            }
...
class Server(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if data['method'] == "ticker":
            ...
            sent_data = MyExchange.Trade(
                access_key, secret_key, pair, order_type, order_side, price, amount
            )
