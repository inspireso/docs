...
cat <<EOF >  /etc/openvpn/easy-rsa/3.0/vars
...
echo '<ca>' >> "$OVPN_TPL"
cat "/etc/openvpn/easy-rsa/3.0/pki/ca.crt" >> "$OVPN_TPL"
echo '</ca>' >> "$OVPN_TPL"

echo '<tls-auth>' >> "$OVPN_TPL"
cat "/etc/openvpn/easy-rsa/3.0/ta.key" >> "$OVPN_TPL"
echo '</tls-auth>' >> "$OVPN_TPL"
