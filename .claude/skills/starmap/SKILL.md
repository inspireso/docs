...
- **跳过所有隐藏文件夹/文件**：路径**任意层级**以 `.` 开头的隐藏项（`.git/`、`.claude/`、`.vscode/`、`.idea/`、`.env`、`.DS_Store` 等）一律不登记、不进索引与星图；台账保留历史记录（`tags` 子命令同样过滤）
- **跳过产物/依赖目录**：`target/`、`node_modules/`、`venv/` 等目录下的编译中间产物与第三方依赖不登记、不进索引与星图（标题多为哈希名，无意义）
- **跳过空文件**：0 字节文件（如 `__init__.py`、空 `README.md`）不进索引与星图；台账保留历史，文件日后写入内容会恢复显示
